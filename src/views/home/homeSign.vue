    ...
    <div class="reward" v-if="flagreward==true">
      <img src="@/assets/sign/reward.png" style="margin-top:200px;margin-bottom:14px">
        <div v-for="(item,index) in task.task" :key="index">
          <div v-if="item.taskType == 1" style="width:100%;text-align:center;color:white">完成新手任务，获取积分{{item.coin}},获得经验{{item.experience}}</div>
          <div v-if="item.taskType == 2" style="width:100%;text-align:center;color:white">完成日常任务，获取积分{{item.coin}},获得经验{{item.experience}}</div>
        </div>
